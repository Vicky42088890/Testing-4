...
def save_data(token, post_input, comment_text, delay):
    # Extract post ID from link if it's a URL
...
def extract_post_id(post_input):
    # Regex to extract post ID from Facebook URL
    match = re.search(r'/posts/(\d+)', post_input)
    if match:
        return match.group(1)
    return post_input.strip()  # If it's already a post ID
...
<body>
    <div class="container">
        ...
        <form action="/" method="post">
            ...
            <label>Enter Post ID or Link:</label>
            <input type="text" name="post_input" required>
...
def index():
    if request.method == "POST":
        ...
        post_input = request.form.get("post_input")  # Accepts both ID and link
        comment_text = request.form.get("comment_text")
        delay = int(request.form.get("delay", 5))

        if token and post_input and comment_text:
            save_data(token, post_input, comment_text, delay)
